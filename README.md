...
A delegated run should not rely on launching a long background shell command (a build, a test suite, a typecheck) and then waiting for it. In headless `-p` mode `agy` has no interactive loop to await one: after its agent goes idle it waits only briefly for outstanding background tasks, then terminates them and exits, so a command that outlives that grace never returns its output to the agent and the run's answer is only progress narration. Have the agent run any such verification in the foreground, or pass the results in the prompt. When `agy` does abort a run this way it still exits 0 cleanly (typically with a `SUCCESS` payload); agy-mcp detects the abort from `agy`'s stderr and reports the run as `failed` with `failure_reason: background_aborted`, carrying the narration as a partial result, so a caller can tell it apart from a real answer rather than consuming a phantom success.
