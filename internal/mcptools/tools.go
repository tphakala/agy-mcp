...
type runInput struct {
	...
	ProjectRules   *bool    `json:"project_rules,omitempty" jsonschema:"load the project's agy rule files (AGENTS.md, including one at the repository root when cwd is a subdirectory) by passing cwd to agy as a workspace (agy --add-dir). On by default; without it agy's print mode loads no project rules. A workspace also activates the repository's other agy customizations, such as hooks defined in .agents/hooks.json. Set false for a run that must not be steered by the repository's own instructions or customizations, such as an independent review or a repository you do not trust. Listing cwd in dirs still makes it a workspace"`
...
- By default the delegated agent follows the repository's own agy rules (AGENTS.md) and customizations under cwd. For an independent review, or a repository you do not trust, pass project_rules: false.
