...
type runInput struct {
	...
	ProjectRules   *bool    `json:"project_rules,omitempty" jsonschema:"load the project's agy rule files (AGENTS.md and the like, found from cwd up to the repository root) by passing cwd to agy as a workspace (agy --add-dir). On by default; without it agy's print mode loads no project rules. Set false for a run that must not be steered by the repository's own instructions, such as an independent review. A workspace also loads the repository's other agy customizations (for example hooks in .agents/hooks.json, per agy's changelog), and false leaves those out too. Listing cwd in dirs still makes it a workspace"`
