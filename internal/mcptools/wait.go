...
type waitInput struct {
	...
	Wait  string `json:"wait,omitempty" jsonschema:"max time to block inline (Go duration, default 90s); a larger value is silently clamped to the ceiling, which stays below common MCP clients' ~120s per-call timeout so a still-running result is delivered inline. Caps only the inline wait, not the job itself: on overrun the job keeps running and can be waited on again or polled with agy_status"`
}
